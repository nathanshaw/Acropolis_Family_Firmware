...
void setupDLManager()
{
  setupDLManagerBasics();
  setupDLManagerRuntime();
  //
  ...
  // clear contents before writing if appropiate
  if (CLEAR_EEPROM_CONTENTS > 0)
  {
    delay(100);
    datalog_manager.clearLogs();
    Serial.println("Clearing the EEPROM Datalog Contents");
  }
  else
  {
...
void setupDLManagerLux()
{
  ...
  if (STATICLOG_LUX_VALUES)
  {
    datalog_manager.addStaticLog("Lowest Lux Recorded : ",
                                 STATICLOG_LUX_MIN_MAX_TIMER, &lux_manager.global_lux_tracker.min_recorded);
    datalog_manager.addStaticLog("Highest Lux Recorded : ",
                                 STATICLOG_LUX_MIN_MAX_TIMER, &lux_manager.global_lux_tracker.max_recorded);
  }
}

void setupDLManagerAutogain()
{
  // Auto Gain
  printMinorDivide();
...
void setupDLManagerRuntime()
{
  Serial.println("configuring datalog_manager timers");
  // datalog_manager.configureAutoTimer(3, DATALOG_START_DELAY_1, DATALOG_TIME_FRAME_1, DATALOG_1_LENGTH);
  //datalog_manager.configureAutoTimer(4, DATALOG_START_DELAY_2, DATALOG_TIME_FRAME_2, DATALOG_2_LENGTH);
  //datalog_manager.configureStaticTimer(2, DATALOG_START_DELAY_3, STATICLOG_RATE_SLOW);
  datalog_manager.configureStaticTimer(3, DATALOG_START_DELAY_1, STATICLOG_RATE_FAST);
  datalog_manager.addStaticLog("Runtime", 3, &runtime);
...
void setupDLManagerCicada()
{
  ...
  setupDLManagerBasics();
  setupDLManagerLux();
  setupDLManagerAutogain();
  setupDLManagerFlashes();
